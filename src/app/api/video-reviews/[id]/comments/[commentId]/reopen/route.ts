...
export async function POST(
  ...
  try {
    ...
    const result = await reopenVideoReviewComment(
      db,
      reviewId,
      commentId,
      profile.id,
    ...
    if (result.changed) {
      const trigger = isReviewAdmin(profile) ? "admin_reopened" : "client_reopened";
      await notifyVideoReviewEvent(trigger, {
        ...
        actorKind: isReviewAdmin(profile) ? "admin" : "client",
      });
    }
    ...
    return NextResponse.json({ error: "Could not reopen comment." }, { status: 500 });
  }
}
